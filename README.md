...
낱개 스킬 10종 + 스킬을 엮는 오케스트레이션 스킬 1종.

| 스킬 | 무엇을 | 검증 게이트 |
|------|--------|-------------|
| `/loop-kit:humanize` | AI 슬롭 제거 + 문체 결 4종 (공식·에세이·대화·SNS) | 슬롭 심각도 등급 + 변경률 가드 |
| `/loop-kit:proofread` | 한국어 교정 (의미 불변, 규범만) | 흔한 맞춤법 오류 잔존 0 + 구두점 |
...
| `/loop-kit:proposal` | 단일 HTML 제안서 (제안 결 3종 : 수주·승인·파트너십) | 문제·솔루션·일정·견적·CTA + 자체완결 |
| `/loop-kit:weekly` | 주간 업무 보고 WBR 6섹션 | 요약·실적·계획·이슈 섹션 + 정량 지표 |
| `/loop-kit:promo` | 교육 홍보 웹페이지 (디자인 결 4종) | 제목·CTA·footer·구두점 검사 |
| `/loop-kit:blog` | 블로그 글 (블로그 결 4종) | H1·H2 2개+·메타·분량 + 구두점 |
| `/loop-kit:email` | 업무 이메일 (이메일 결 4종) | 제목·요청·서명·분량 + 구두점 |
| `/loop-kit:resume` | 단일 HTML 이력서·자기소개서 (이력 결 3종) | 연락처·경력·역량·정량 성과 + 자체완결 |
| `/loop-kit:loop` | 스킬을 엮는 오케스트레이션 (소스→검수→변환→검증→승인→싱크) | 루프 정의 린터 : 빈 계약·6컬럼·사람 체크포인트·구두점 |

> 현재 `v0.10.0` : 스킬 11종. Tier 1 한국 실무 스킬 6종(proposal·email·weekly·blog·proofread·resume) + 오케스트레이션 스킬 loop 추가.
